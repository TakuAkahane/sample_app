class CreateBuyResidentialProperties < ActiveRecord::Migration[5.2]
  def change
    create_table :buy_residential_properties do |t|
      t.string :name, comment: '物件名', :null => false
      t.integer :ward_id, comment: '区ID', :null => false
      t.string :address, comment: '住所', :null => false
      t.integer :price, comment: '金額', :null => false
      t.string :floore_plan, comment: '間取り', :null => false
      t.integer :floore_size, comment: '専有面積', :null => false
